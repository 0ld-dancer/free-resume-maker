...
// Initialize using the modern SDK pattern
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

// Set your modern production model string
...
export async function analyzeResume(resumeData) {
  ...
  try {
    // Modern syntax uses ai.models.generateContent directly
...
export async function getSuggestions(section, currentContent) {
  ...
  try {
    ...
    if (error.message?.includes('quota') || error.status === 429) {
      throw new Error("QUOTA_ERROR: API quota exceeded or Project Limit set to 0.");
    }
    if (error.message?.includes('invalid') || error.status === 403) {
      throw new Error("API_ERROR: Invalid API key configuration");
    }
